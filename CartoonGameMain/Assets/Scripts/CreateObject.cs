...
public class CreateObject : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 20; i <= 220; i += 10)
        {
            ...
            Instantiate(cheese, new Vector3(posX[randomCoin], 0f, i + 5), Quaternion.identity);
    ...
    private void createObject()
    {
        for (int j = 20; j <= 200; j += 10)
        {
            ...
            Instantiate(cheese, new Vector3(posX[randomCoin], 0f, j + lastBarrier + 5), Quaternion.identity);
            if (j == 200)
            {
                lastBarrier += j;
            }
